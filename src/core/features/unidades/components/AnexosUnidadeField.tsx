// src/features/unidades/components/AnexosUnidadeField.tsx
...
interface AnexosUnidadeFieldProps {
  /** Nulo enquanto a instalacao nao foi criada. */
  unidadeId: string | null;
  somenteLeitura?: boolean;
  /**
   * Permite enviar os arquivos escolhidos antes de a instalacao existir,
   * assim que o id nascer. Sem isso, cadastrar com o contrato em maos exigiria
   * salvar, reabrir o sheet e anexar.
   */
  registrarAcaoPosCriacao?: (
    chave: string,
    acao: (unidadeId: string) => Promise<void>,
  ) => void;
}
...
/**
 * Documentos da instalação: contrato de fornecimento, diagrama unifilar,
 * laudo, ART.
 *
 * Nativo do sheet, e não injetado pelo consumidor: o backend vive no api-shared
 * e os dois produtos falam com ele, então não há o que particularizar por app.
 *
 * O download passa pelo controller (`/unidades/anexos/:id/download`) e não por
 * URL estática. Isso evita a armadilha da foto do equipamento, que depende de
 * uma rota no UploadsController duplicada nos dois backends — e sem ela responde
 * 404 sem aparecer nos logs.
 */
export function AnexosUnidadeField({
  unidadeId,
  somenteLeitura = false,
  registrarAcaoPosCriacao,
}: AnexosUnidadeFieldProps) {
  const httpClient = useHttpClient();
  const inputRef = useRef<HTMLInputElement>(null);

  const [anexos, setAnexos] = useState<AnexoUnidade[]>([]);
  ...
  const carregar = useCallback(async () => {
    ...
    setCarregando(true);
    try {
      const resposta = await httpClient.get(`/unidades/${id}/anexos`);
  ...
  useEffect(() => {
    ...
    registrarAcaoPosCriacao('os anexos', async (novoId: string) => {
      for (const arquivo of pendentes) {
        ...
        await httpClient.post(`/unidades/${novoId}/anexos`, form, {
  ...
  const enviarArquivos = async (arquivos: FileList) => {
    ...
    try {
      ...
      for (const arquivo of Array.from(arquivos)) {
        ...
        form.append('file', arquivo);
        try {
          await httpClient.post(`/unidades/${id}/anexos`, form, {
            // O Content-Type vai explicito: a instancia do axios define
            // application/json por padrao, e sem sobrescrever aqui o multer nao
            // reconhece o corpo como multipart — o arquivo chega vazio e o
            // backend recusa com "Nenhum arquivo enviado".
  ...
  const baixar = async (anexo: AnexoUnidade) => {
    try {
      const resposta = await httpClient.get(`/unidades/anexos/${anexo.id.trim()}/download`, {
        responseType: 'blob',
      });
  ...
  const remover = async (anexo: AnexoUnidade) => {
    if (!confirm(`Remover "${anexo.nome_original}"?`)) return;

    setRemovendo(anexo.id);
    try {
      await httpClient.delete(`/unidades/anexos/${anexo.id.trim()}`);
    ...
    <div className="space-y-3">
      ...
      {carregando ? (
        ...
        <p className="text-sm text-muted-foreground">
          Nenhum anexo. Contrato, diagrama, laudo e documentos desta instalação ficam aqui.
        </p>
      ) : (
        <div>
          {anexos.map((anexo) => (

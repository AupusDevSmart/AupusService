...
interface OrigemOSSelectorProps {
  value: {
    tipo: 'ANOMALIA' | 'PLANO_MANUTENCAO' | 'SOLICITACAO_SERVICO' | 'MANUAL';
    anomaliaId?: string;
    planoId?: string;
    solicitacaoServicoId?: string;
    tarefasSelecionadas?: string[];
    // Campos de hierarquia (Planta → Unidade → Anomalia/Plano/Solicitação)
    plantaId?: string;
    unidadeId?: string;
...
export const OrigemOSSelector: React.FC<OrigemOSSelectorProps> = ({
  ...
  // Carregar anomalias/solicitações quando planta e unidade forem selecionadas
  useEffect(() => {
    ...
    } else if (tipo === 'SOLICITACAO_SERVICO' && plantaId && unidadeId) {
      console.log('🔍 [OrigemOSSelector] Carregando solicitações filtradas:', { plantaId, unidadeId });
      carregarSolicitacoes(plantaId, unidadeId);
    }
  }, [tipo, plantaId, unidadeId, carregarAnomalias, carregarSolicitacoes]);
  ...
  const handleTipoChange = (tipo: 'ANOMALIA' | 'PLANO_MANUTENCAO' | 'SOLICITACAO_SERVICO' | 'MANUAL') => {
  ...
  const handleSolicitacaoChange = (solicitacaoId: string) => {
    console.log('🔍 [OrigemOSSelector] Selecionando solicitação:', solicitacaoId);

    // Se string vazia, limpar seleção
    if (!solicitacaoId || solicitacaoId === '') {
      const novoValor = {
        ...value,
        solicitacaoServicoId: undefined
      };
    ...
    const solicitacaoSelecionada = solicitacoesDisponiveis.find(s => String(s.id) === String(solicitacaoId));
    ...
    const novoValor = {
      ...
      plantaId: value.plantaId,
      unidadeId: value.unidadeId,
      solicitacaoServicoId: String(solicitacaoId),
      anomaliaId: undefined,
      planoId: undefined,
      tarefasSelecionadas: []
    };
    ...
    // Notificar local/ativo
    if (onLocalAtivoChange && solicitacaoSelecionada) {
      onLocalAtivoChange(solicitacaoSelecionada.local, solicitacaoSelecionada.local);
    }
  };
  ...
  const solicitacoesFiltradas = solicitacoesDisponiveis.filter(solicitacao =>
  ...
  const renderPlantaSelector = () => {
    ...
    if (tipo === 'PLANO_MANUTENCAO') return null; // Planos não usam hierarquia planta/unidade
  ...
  const renderUnidadeSelector = () => {
    if (tipo === 'MANUAL' || tipo === 'PLANO_MANUTENCAO' || !plantaId) return null;
  ...
  const renderTipoSelector = () => (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Button
          type="button"
          variant={tipo === 'SOLICITACAO_SERVICO' ? 'default' : 'outline'}
          className={`h-24 flex-col gap-2 relative transition-all duration-200 ${
            tipo === 'SOLICITACAO_SERVICO'
              ? 'ring-2 ring-primary ring-offset-2 bg-primary hover:bg-primary/90'
              : 'hover:border-primary/50 hover:bg-primary/5'
          }`}
          onClick={() => handleTipoChange('SOLICITACAO_SERVICO')}
          disabled={disabled}
        >
          <FilePenLine className={`h-6 w-6 ${tipo === 'SOLICITACAO_SERVICO' ? 'text-primary-foreground' : 'text-orange-600'}`} />
  ...
  const renderSolicitacaoSelector = () => {
    // Só mostrar se planta e unidade estiverem selecionadas
    if (!plantaId || !unidadeId) {
      return (
        <div className="text-center py-8 text-muted-foreground border border-dashed border-border rounded-lg bg-muted/30">
    ...
    const solicitacaoSelecionada = solicitacoesDisponiveis.find(s => String(s.id).trim() === solicitacaoServicoId);
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          {!solicitacaoSelecionada && (
            <span className="text-xs text-muted-foreground">
              {solicitacoesDisponiveis.length} disponíveis
            </span>
          )}
        </div>
            ...
            <CardContent className="p-3">
              <div className="flex items-start justify-between">
                ...
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  onClick={() => handleSolicitacaoChange('')}
                  disabled={disabled}
                >
                  Trocar
                </Button>
          ...
          <>
            ...
            <div className="max-h-64 overflow-y-auto space-y-2 border border-border rounded-lg p-2 bg-muted/30">
              {solicitacoesFiltradas.map((solicitacao) => {
                const isSelected = solicitacaoServicoId === String(solicitacao.id).trim();
                ...
                return (
                  <Card
                    ...
                    onClick={() => {
                      if (!disabled) {
                        handleSolicitacaoChange(String(solicitacao.id));
                      }
                    }}
                  >
                    <CardContent className="p-3">
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ✅ HIERARQUIA COM BREADCRUMB VISUAL - SOLICITACAO_SERVICO */}
      {tipo === 'SOLICITACAO_SERVICO' && (
        <>
          {/* Breadcrumb de progresso */}
          <div className="flex items-center justify-center gap-2 py-2">
            ...
            <div className="flex items-center gap-2">
              <div className={`flex items-center justify-center w-8 h-8 rounded-full text-xs font-medium transition-colors ${
                solicitacaoServicoId ? 'bg-primary text-primary-foreground' : unidadeId ? 'bg-muted text-muted-foreground' : 'bg-muted/50 text-muted-foreground/50'
              }`}>
                3
              </div>
              <span className={`text-xs font-medium ${
                solicitacaoServicoId ? 'text-foreground' : unidadeId ? 'text-muted-foreground' : 'text-muted-foreground/50'
              }`}>
                Solicitação
              </span>
            </div>
          ...
          <div className="space-y-4">
            ...
            {/* Passo 3: Selecionar Solicitação */}
            {plantaId && unidadeId && (
              <div className={`rounded-lg border-2 transition-all duration-200 ${
                unidadeId && !solicitacaoServicoId ? 'border-primary bg-primary/5' : 'border-border bg-background'
              }`}>
                <div className="p-4">
                  {renderSolicitacaoSelector()}
                </div>
              </div>
            )}
          </div>
          ...
          {solicitacaoServicoId && (
            <div className="p-3 bg-primary/10 border border-primary/20 rounded-lg">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-primary" />
                <p className="text-sm text-primary font-medium">
                  Solicitação selecionada com sucesso
                </p>
              </div>
            </div>
          )}

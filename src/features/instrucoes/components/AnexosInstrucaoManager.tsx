// src/features/instrucoes/components/AnexosInstrucaoManager.tsx
...
import { AnexoInstrucaoApiResponse, TipoAnexo } from '@/services/instrucoes.services';

interface AnexosInstrucaoManagerProps {
  instrucaoId: string | null;
  readonly?: boolean;
  onFilesChange?: (files: File[]) => void;
}

export function AnexosInstrucaoManager({ instrucaoId, readonly = false, onFilesChange }: AnexosInstrucaoManagerProps) {
  const [anexos, setAnexos] = useState<AnexoInstrucaoApiResponse[]>([]);
  const [localFiles, setLocalFiles] = useState<File[]>([]);
  const [loading, setLoading] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const { getAnexos, uploadAnexo, downloadAnexo, deleteAnexo } = useInstrucoesApi();
  const isCreateMode = instrucaoId === null;

  useEffect(() => {
    if (instrucaoId) loadAnexos();
  }, [instrucaoId]);
  ...
  const loadAnexos = async () => {
    try {
      setLoading(true);
      setError(null);
      const lista = await getAnexos(instrucaoId!);
      setAnexos(lista);
    } catch (err: any) {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const allowedTypes = [
      'image/png', 'application/pdf', 'image/jpeg', 'image/jpg',
  ...
  const handleDownload = async (anexo: AnexoInstrucaoApiResponse) => {
    try {
      const blob = await downloadAnexo(instrucaoId!, anexo.id);
  ...
  const handleDelete = async (anexo: AnexoInstrucaoApiResponse) => {
    if (!confirm(`Tem certeza que deseja excluir o anexo "${anexo.nome}"?`)) return;
    try {
      await deleteAnexo(instrucaoId!, anexo.id);
      await loadAnexos();
    } catch (err: any) {
              ...
              <div key={anexo.id} className="flex items-center justify-between p-3 bg-muted/50 border rounded-md hover:bg-muted/80">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Badge variant="secondary" className="text-xs">{getTipoLabel(anexo.tipo)}</Badge>
                      {anexo.tamanho && (
